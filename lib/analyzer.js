...
const topicKeywords = {
  ...
  healthcare: ["의료", "의료기관", "병원", "의원", "환자", "진료", "요양", "응급"],
  fire: ["소방", "소방법", "소방시설", "스프링클러", "화재", "피난", "방화", "소화설비"],
...
const sectorKeywords = {
  ...
  healthcare: ["의료", "의료기관", "병원", "의원", "환자", "진료", "요양", "응급"],
  fire: ["소방", "소방법", "소방시설", "스프링클러", "화재", "피난", "방화"],
};
...
const queryExpansionRules = [
  ...
  },
  {
    ...
    queries: ["의료법", "의료기관", "응급의료에 관한 법률"],
  },
  {
    terms: ["건축", "건물", "시설", "용도변경", "사용승인"],
    queries: ["건축법", "건축물관리법"],
  },
  {
  ...
  },
  {
    ...
    queries: ["전자상거래 등에서의 소비자보호에 관한 법률", "표시 광고의 공정화에 관한 법률"],
  },
  {
...
export function hydrateAnalysisWithLiveLaws(analysis, lawApi, mode = "impact") {
  const liveLaws = buildLiveLaws(lawApi?.items || [], analysis.conditions);
  if (!liveLaws.length) return analysis;

  const localHighConfidence = analysis.laws.filter((law) => law.score >= 60);
  const laws = mergeLaws(liveLaws, localHighConfidence).slice(0, 12);
...
function buildSearchQueries(query, conditions, laws) {
  ...
  const values = [
    ...
    ...expandSearchQueries(query),
    ...extractSearchTerms(query).slice(0, 5),
    ...conditions.topics.map((topic) => topicSearchFallback[topic]).filter(Boolean),
    ...highConfidenceLawNames.slice(0, 3),
  ];
  return unique(values.map(cleanSearchQuery).filter(Boolean)).slice(0, 8);
}
...
function buildLiveLaws(items, conditions) {
  return items
    .filter((item) => item?.title)
    .map((item, index) => {
      const title = item.title || "법령명 없음";
      const inferredTopics = inferTopics(`${title} ${item.matchedQuery || ""}`);
      const topics = unique([...(conditions.topics || []), ...inferredTopics]);
      ...
      return {
        ...
        title,
        type: "법제처",
        article: item.enforcementDate ? `시행 ${formatDate(item.enforcementDate)}` : "전체 법령 검색 결과",
        summary: `${item.matchedQuery || title} 검색으로 확인된 법제처 법령`,
        evidence: `법제처 전체 법령 검색에서 "${item.matchedQuery || title}" 검색어로 확인된 법령입니다. 적용 조문, 별표, 부칙은 원문에서 확인하세요.`,
        agency: item.agency || "소관부처 미확인",
        sectors: unique([conditions.sector, "general"]).filter(Boolean),
        ...
        tasks: [`${title} 원문 조문 확인`, `${title} 시행일·개정이력 확인`],
        relations: [],
        score: Math.max(55, 98 - index * 3),
        source: "lawApi",
