...
export function hydrateAnalysisWithLiveLaws(analysis, lawApi, mode = "impact") {
  ...
  const laws = prioritizeDisplayLaws(mergeLaws(liveLaws, [...localHighConfidence, ...queryAnchoredLocal]), analysis.searchQueries).slice(0, 12);
...
function prioritizeDisplayLaws(laws, searchQueries = []) {
  ...
  queries.forEach((query) => {
    ...
    if (match && scoreLawTitleForQuery(match.title, query) >= 55) {
      picked.push(match);
      seen.add(normalize(match.title));
    }
  });

  const remainder = [...laws]
    .filter((law) => !seen.has(normalize(law.title)))
    .sort((left, right) => (right.score || 0) - (left.score || 0));

  const decreeCount = new Map();
  return [...picked, ...remainder].filter((law) => {
