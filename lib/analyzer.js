...
export function hydrateAnalysisWithLiveLaws(analysis, lawApi, mode = "impact") {
  ...
  if (!liveLaws.length && !localHighConfidence.length && !queryAnchoredLocal.length) {
    return analysis;
  }

  const laws = prioritizeDisplayLaws(
    mergeLaws(liveLaws, [...localHighConfidence, ...queryAnchoredLocal]),
...
function inferFallbackTopics(text) {
  ...
  if (/(인허가|허가|신고|등록|창업|개업|영업신고|설립)/u.test(text)) {
    return ["permit"];
  }
  return ["permit"];
}
