...
export function isGarbageLawQuery(value) {
  const text = String(value || "").trim();
  const normalized = normalizeCompact(text);
  if (!text) return true;
  if (["법령", "관련법령", "그냥법", "어떤법", "무슨법", "어느법"].includes(normalized)) return true;
  if (/^(그냥|어떤|무슨|어느|뭐|좀|이상한|알려|궁금).{0,8}법$/u.test(normalized)) return true;
  if (normalized.length <= 4 && normalized.endsWith("법")) return true;
  return false;
}
