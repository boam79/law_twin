...
import { getLawApiMaxQueries } from "./lawApiConfig.js";
...
const MIN_TITLE_MATCH_SCORE = 35;
...
export async function fetchLawSearchResults(query) {
  ...
  try {
    ...
    const ranked = rankLawSearchItems(normalizeLawItems(data?.LawSearch?.law), query);
    const matched = ranked.filter((item) => scoreLawItemForQuery(item, query) >= MIN_TITLE_MATCH_SCORE);
    const rawItems = matched.length ? matched : ranked.slice(0, 3);
...
export async function fetchLawSearchBatch(queries, options = {}) {
  const emergencyQueries = options.emergencyQueries || [];
  const primary = await fetchLawSearchBatchInternal(queries);

  if (!primary.enabled || primary.items.length > 0 || !emergencyQueries.length) {
    return primary;
  }

  const retryQueries = [
    ...new Set(
      [...(primary.queries || []), ...emergencyQueries]
        .map((query) => canonicalizeLawQuery(String(query || "").trim()))
        .filter(Boolean),
    ),
  ].slice(0, getLawApiMaxQueries());

  const retry = await fetchLawSearchBatchInternal(retryQueries);
  return mergeLawSearchBatches(primary, retry);
}

async function fetchLawSearchBatchInternal(queries) {
  const cleanQueries = [
  ...
  return {
    ...
    errors,
    error: errors.length === results.length && results.length > 0 ? errors.map((item) => `${item.query}: ${item.message}`).join(" / ") : null,
  };
}

function mergeLawSearchBatches(primary, secondary) {
  const itemsByKey = new Map();
  ...
  const queries = [...new Set([...(primary.queries || []), ...(secondary.queries || [])])];
  ...
  return {
    ...
    errors,
    error:
      itemsByKey.size === 0 && errors.length
        ? errors.map((item) => `${item.query}: ${item.message}`).join(" / ")
...
function rankLawSearchItems(items, query) {
  return [...items].sort((left, right) => {
    ...
    const leftScore = scoreLawItemForQuery(left, query);
    const rightScore = scoreLawItemForQuery(right, query);
    if (rightScore !== leftScore) return rightScore - leftScore;
    return String(leftTitle).length - String(rightTitle).length;
  });
}

function scoreLawItemForQuery(item, query) {
  const title = item["법령명한글"] || item.lawName || item.title || "";
  return scoreLawTitleForQuery(title, query);
}
