...
export async function planLawSearchWithGemini({ scenario, analysis }) {
  ...
  try {
    ...
    if (!response.ok) {
      return buildPlanFallback(fallbackQueries, redactSecrets(`Gemini API ${response.status}`));
    }
...
export async function summarizeWithGemini({ scenario, analysis, lawApi }) {
  ...
  try {
    ...
    if (!response.ok) {
      return { enabled: true, configured: true, text: null, error: redactSecrets(`Gemini API ${response.status}`) };
    }
    ...
    return {
      ...
      configured: true,
      text: null,
      error: redactSecrets(error instanceof Error ? error.message : "Gemini API 호출 실패"),
    };
  }
}
  ...
  for (const delay of attempts) {
    ...
    try {
      ...
      if (response.status === 429 && delay === 0) {
        lastError = new Error("Gemini API 429");
        continue;
      }
