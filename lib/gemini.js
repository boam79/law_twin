...
export async function planLawSearchWithGemini({ scenario, analysis }) {
  ...
  try {
    ...
    if (!response.ok) {
      return buildPlanFallback(fallbackQueries, `Gemini API ${response.status}`);
    }
    ...
    return buildPlanFallback(fallbackQueries, error instanceof Error ? error.message : "Gemini 검색 계획 실패");
  }
}
...
export async function summarizeWithGemini({ scenario, analysis, lawApi }) {
  ...
  try {
    ...
    if (!response.ok) {
      return { enabled: true, configured: true, text: null, error: `Gemini API ${response.status}` };
    }
