import { expandLawAliasQueries, ignoredStandaloneLawQueries, isLawLikeSearchQuery, normalizeCompact } from "./lawSearchTerms";
...
function normalizeQueries(value) {
  ...
  return [...new Set(value.flatMap(expandLawAliasQueries).map((item) => String(item || "").trim()).filter(Boolean))]
