...
import { getGeminiModel, getGeminiModelChain, redactSecrets, wrapScenarioForPrompt } from "./security.js";

const GEMINI_ENDPOINT = "https://generativelanguage.googleapis.com/v1beta/models";
export const GEMINI_RATE_LIMIT_HINT =
  "무료 Gemini 모델 한도(분당 약 5회)에 모두 걸렸습니다. 1~2분 기다린 뒤 다시 분석해 주세요. 체크리스트·법령 결과는 아래에서 계속 볼 수 있습니다.";
...
async function generateContentWithModelFallback({ apiKey, prompt, temperature, maxOutputTokens }) {
  ...
  for (const model of models) {
    ...
    if (response.ok) {
      return { ...response, model, modelsTried };
    }

    lastStatus = response.status;
    if (response.status !== 429) {
      return { ...response, model, modelsTried, allRateLimited: false };
    }
  }
...
function buildModelFallbackNote(response) {
  ...
  return `${first} 한도로 ${used} 모델로 요약했습니다.`;
}
