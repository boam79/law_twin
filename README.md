...
**현재 버전:** `0.5.5` · **프로덕션:** [law-twin.vercel.app](https://law-twin.vercel.app)
...
| `GEMINI_PLANNER_MODE` | *(예정)* `on`이면 법령 검색어 설계에 Gemini 1회 (`planLawSearchWithGemini`) |
| `GEMINI_PLANNER_MODEL` | *(예정)* Planner 기본 모델 (권장 `gemini-2.5-flash-lite`) |
| `GEMINI_PLANNER_MODEL_FALLBACK` | *(예정)* 404·5xx만: `gemini-2.5-flash,gemini-2.5-pro` |

### Gemini Planner 모드 (설계·미구현)
...
- **분석당 Gemini 1회** 유지: Planner `on`이면 `GEMINI_SUMMARY_MODE=off` 권장.
- **429** 시 추가 모델 시도 없음 (요약·Planner 공통).
- 구현 코드: `planLawSearchWithGemini` (`lib/gemini.js`) — 현재 API는 내부 검색어만 사용.
