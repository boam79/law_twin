...
**현재 버전:** `0.5.1` · **프로덕션:** [law-twin.vercel.app](https://law-twin.vercel.app)
...
| `GEMINI_MODEL` | 요약에 우선 사용할 모델 (기본 `gemini-2.5-flash`) |
| `GEMINI_MODEL_FALLBACK` | 429 시 순차 시도할 모델 목록(쉼표 구분) |
...
- 분석당 Gemini 호출 **1회**(검색 계획은 내부 검색어 사용)
- **429** 시 모델 폴백: Flash → Flash Lite → 2.0 Flash → 2.5 Pro
- 전 모델 한도 시 **75초** UI 쿨다운 및 안내 메시지
- 체크리스트·법령·히트맵은 요약 실패 시에도 표시
...
- `GEMINI_MODEL_FALLBACK` / `getGeminiModelChain()` — 429 시 Flash·Flash Lite·2.0 Flash·Pro 순 시도
